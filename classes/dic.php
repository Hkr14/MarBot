...
$dicresult = urlencode("<b>═════════ 『 Team Mar® 』═════════
•• Word: $dict
•Meanings : 
•1:$definition0
•2:$definition1
...
if(empty($client)){
 ...
 exit();
  ...
  if(empty($git)){
reply_to($chatId,$message_id,$keyboard,"<b>GIVE ME A USER NAME %0AEX: <code>!git Sarcehkr</code></b>");
exit();
}
   $curl = curl_init();
   curl_setopt_array($curl, [
...
if ($gusername) {
        reply_to($chatId,$message_id,$keyboard,$gitresult);
           ...
           reply_to($chatId,$message_id,$keyboard,"<b>Sorry! User Not Found %0AInvalid github username  %0AEX: <code>!git Sarcehkr</code></b>");
}
    }
